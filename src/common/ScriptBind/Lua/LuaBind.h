...
#define MAKE_VOID_FUNCTIONCLASSX(N) \
	...
	inline void RegisterClass(lua_State* L, const char* name) 
	{
		...
		{
			log_error("can't not find lua table:%s\n", name);
		}
	...
	inline void RegisterMethod(lua_State* L, const char* name, GenericMethod* method) 
	{
		...
		{
			log_error("please register class %s before regist method\n",  ScriptMetaClass<T>::GetName());
		}
		lua_pop(L, 1);
	}
	...
	inline void RegisterStaticMethod(lua_State* L, const char* name, GenericFunction* func) 
	{
		...
		{
			log_error("please register class %s before regist static method\n",  ScriptMetaClass<T>::GetName());
		}
		lua_pop(L, 1);
	}
	...
	inline static void ReleaseAllCaches()
	{
		for (std::vector<GenericFunction*>::iterator itr = gCachedFunctions.begin(); itr != gCachedFunctions.end(); ++itr)
			delete *itr;
		gCachedFunctions.clear();

		for (std::vector<GenericMethod*>::iterator itr = gCachedMethods.begin(); itr != gCachedMethods.end(); ++itr)
			delete *itr;
		gCachedMethods.clear();

		for (std::vector<ClassMetaData*>::iterator itr = gCachedMetaDatas.begin(); itr != gCachedMetaDatas.end(); ++itr)
